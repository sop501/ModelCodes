...
public class ModelClass1 {
	public static void main(String [] args) throws Exception{
		...
		//create a new model
		EmfModel model= createEmfModel("Model", "student.ecore","Ecore.ecore", true, false);
		EmfModel model2= createEmfModel("Model2", "student.model","student.ecore", true, true);
		...
		for (Operation operation: module.getOperations()){
			System.out.println("fsd  "+operation.getContextTypeExpression().getName());
				...
				if(clas instanceof EClass){
					...
					if(!(eclass.isAbstract())){
						...
						for (Operation operation: operationNames){
							...
							if(operation.getContextTypeExpression().getName().equals(eclass.getName())){					
								annotationBlock = operation.getAnnotationBlock();
	...
	protected static EmfModel createEmfModel(String name, String model, 
		...
		System.out.println(getFile(metamodel).toURI().toString());
		return emfModel;
	}
	protected static File getFile(String fileName) throws URISyntaxException {
		
		URI binUri = ModelClass1.class.
