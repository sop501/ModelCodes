...
public class ModelClassNewObject {
	...
	protected void executeModule(File ecoreFile,File eolFile) throws Exception{
		...
		String ecoreN= ecoreNew.toString()+"frt.ecore";
		...
		EmfModel model2= executeOperations(module.getOperations(),model1);
	...
	protected EmfModel executeOperations(ArrayList<Operation> operationNames,EmfModel model2) throws EolModelElementTypeNotFoundException, EolRuntimeException{
			...
			for(Annotation annotation:annotationBlock.getAnnotations()){
				...
				if(name.equals("create")){
					...
					if (!annotationValues.isEmpty()) {
						ann=(String) annotationValues.get(0);
						EClass refClas;
						EStructuralFeature refClass=getFeature(ann);
						/*if(ann.contains("(")){
	...
	protected EStructuralFeature getFeature(String string){
		EStructuralFeature feature;
		EClass refClas;
		String temp,temp1,temp2,temp3,property;
		//property="";
		int index;
		String[] tempArray;
		feature=EcoreFactory.eINSTANCE.createEReference();
		if(string.contains("(")){ //check if it has parameters
			tempArray= string.split("\\(", 2);
			//temp=tempArray[0].trim();
			//refClas = model2.classForName();
			...
			if(string.contains("annotation")){  //check if the parameters include annotations
				tempArray= string.split("annotation");
				property=tempArray[0];
				for(int i=1;i<tempArray.length;i++){
					if(tempArray[i].contains("(")){
						if(tempArray[i].contains(")")){
							string=tempArray[i].trim();
							index=string.indexOf(")");
							temp2=string.substring(1,index+1);
							feature.getEAnnotations().add(getAnnotation(temp2));
						...
						else{ //if it has opening bracket, then there must be a closing bracket
							System.out.println("error");
						}
					
					}
				
					else{ //the annotation has only source and no details
						if(tempArray[i].contains(",")){
							string=tempArray[i].trim();
							index=string.indexOf(",");
						...
						else{ //no comma means it is at the end of the parameters
							string=tempArray[i].trim();
		...
		else { //if no parameter is specified
			/*if(string.contains("(")){// if the string does not contain annotations
			tempArray= string.split("\\(", 2);
			//temp=tempArray[0].trim();
			//refClas = model2.classForName();
			temp=tempArray[1].trim();
			property=temp.substring(0, temp.length()-1);
			//refClass=getFeature(temp2[1]);
			//referenceMap.put(temp2[0].trim(), refClass);
			}
			else*/
				return null;// it must have a type
		}
		...
		Map<String,String> map = getProperties(property);
		if (map.containsKey("type")){
			temp= map.get("type");
			if (classes.containsKey(temp)){
				feature.setEType(classes.get(temp));
			}
			else
				return null;
		...
		if(map.containsKey("name"))
			...
			feature.setName(random.generateString());
		
		if (map.containsKey("containment")){
			temp= map.get("containment");
			if (map.get("containment").equals("true")){
				((EReference) feature).setContainment(true);
			}
	...
	public static void main(String[] args){
		ModelClassNewObject model= new ModelClassNewObject();
